...
<Steps>
  <Step title="Write the component">
    ...
    - No literal colours, and no chroma. If a value cannot be expressed as a token or a grey, it does not belong — see [theming](/docs/theming).
    - No gradients and no shadows. Separation is drawn with a rule.
    - Square corners. The radius scale is zero, so do not reach for `rounded-*` to get a shape back.
...
- No hard-coded colours, no gradients, no shadows, no `rounded-*` used to
  reintroduce a radius.
