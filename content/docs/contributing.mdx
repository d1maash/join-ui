...
<Steps>
  <Step title="Write the component">
    ...
    - No gradients and no shadows. Separation is drawn with a rule.
    - Round through `rounded-soft-sm`, `rounded-soft` and `rounded-soft-lg`. The chrome scale is still zero, so a plain `rounded-*` is a no-op.
...
`Callout` signals severity with an icon and a spelled-out label, and thickens
its left rule for the two serious variants. Nothing about it depends on hue, so
do not add a variant expecting a colour.
