...
import { CardGroup, DocsCard } from "@/components/docs/card-group"
...
function heading(level: 1 | 2 | 3 | 4) {
  ...
  const sizes = {
    ...
    2: "mt-10 mb-3 scroll-mt-24 border-t border-border pt-8 text-xl font-semibold tracking-tight first:mt-0 first:border-0 first:pt-0",
    3: "mt-7 mb-2 scroll-mt-24 text-base font-semibold tracking-tight",
...
function InstallTabs({ target, label }: { target: string; label?: string }) {
  return <PackageManagerTabs commands={shadcnCommands(target)} label={label} />
}
...
export const mdxComponents = {
  ...
  a: ({ href = "", className, ...props }: React.ComponentPropsWithoutRef<"a">) => {
    ...
    const style = cn(
      "rounded-sm font-medium text-primary underline underline-offset-4 transition-colors hover:text-foreground",
  ...
  ol: ({ className, ...props }: React.ComponentPropsWithoutRef<"ol">) => (
    <ol
      className={cn(
        "my-4 flex list-decimal flex-col gap-2 pl-5 text-muted-foreground marker:text-muted-foreground",
        className
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: React.ComponentPropsWithoutRef<"code">) => (
    <code
      className={cn(
        "rounded border border-border bg-muted px-[0.3em] py-[0.15em] font-mono text-[0.85em] text-foreground",
        className
      )}
      {...props}
    />
  ...
  th: ({ className, ...props }: React.ComponentPropsWithoutRef<"th">) => (
    <th
      scope="col"
      className={cn(
        "border-b border-border bg-muted/50 px-4 py-2.5 font-medium",
        className
      )}
      {...props}
    />
